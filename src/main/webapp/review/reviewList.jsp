...
<%
	ArrayList<reviewDTO> list = dao.reviewList(start,end);
	if(list.size() > 0 ){
		for(reviewDTO dto : list){%>
		<tr>
			<td><%=dto.getRenum()%></td>
			<td><a href="/ProTwo/review/reviewContent.jsp?renum=<%=dto.getRenum()%>"><%=dto.getTitle()%></a>
<%				
			boolean result = dao.buyCheck(dto.getId() , dto.getItemname());
			int goodcount = dao.goodcount(dto.getRenum());
			if(result == true){ %></td>
			<td>(구매)<%=dto.getId()%></td>
<%			}else{%>
			<td><%=dto.getId()%></td>
