...
<form name="frm" action="/ProTwo/myPage/updatePro.jsp" method="post" onsubmit="return checkValue();">
	아이디 : <%=dto.getId()%> <br />
			<input type="hidden" name="id" value="<%=dto.getId()%>" />
	비밀번호 : <input type="password" name="pw" placeholder="비밀번호를 입력하세요." value="" onKeyPress="capsLock(event)" onkeyup="noSpaceForm(this);" onchange="noSpaceForm(this);"/> 
			<input type="button" value="비밀번호 변경" onclick="window.location='/ProTwo/myPage/pwChangeForm.jsp'" /> <br />
			<div id="pwCheck"></div>
	이  름 : <%=dto.getName()%> <br />
	...
	주  소 : <%=dto.getAdrs()%>
		<input type="hidden" value=<%=dto.getAdrs()%> name="adrs" />
			<input type="button" value="주소 변경" onclick="window.location='/ProTwo/myPage/updateAdrs.jsp'" /><br />
	...
	비밀번호 찾기 답 : <input type="text" name="ans" value="<%=dto.getAns()%>" onkeyup="noSpaceForm(this);" onchange="noSpaceForm(this);"/> <br />
		...
		<input type="button" value="회원탈퇴" onclick="window.location='/ProTwo/myPage/deleteForm.jsp' " />
</form>
<br/>
<br/>	
